...
# NOTE: Anki configuration is split between two classes: AnkiConfig (note-invariant) and AnkiNoteConfig (note-specific).
# To check a snippet, we need one of each. No merging involved.
# Directory configuration is in one class: DirectoryConfig.
# To check a snippet, we just need an instance. But it may be generated by merging multiple instances.
# TLDR: Anki and directory configurations are managed quite differently.
...
@dataclass(frozen=True)
class AnkiConfig:
    """Note-invariant information required for checking Anki notes.

    To check a note, we require note-specific information too.
    """
...
class AnkiNoteConfig:
    """
    Note-specific information required for checking an anki note.

    I.e. information that can vary across notes.
    """
