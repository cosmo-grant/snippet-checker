...
class TestPythonFormat:
    def test_no_change(self):
        code = 'print("hello")\n'
        snippet = PythonSnippet(code, "python:3.13", "test-ruff")
...
class TestGoFormat:
    def test_no_change(self):
        code = 'package main\n\nimport "fmt"\n\nfunc main() {\n\tfmt.Println("hello")\n}\n'
        snippet = GoSnippet(code, "golang:1.24", "test-gofmt")
        assert snippet.format(compress=False) == code
...
class TestNodeFormat:
    def test_no_change(self):
        ...
        snippet = NodeSnippet(code, "node:24", "test-prettier")
        assert snippet.format(compress=False) == code
...
class TestRubyFormat:
    def test_no_change(self):
        code = "puts 'hello'\n"
        snippet = RubySnippet(code, "ruby:3.4", "test-rubocop")
        assert snippet.format(compress=False) == code
...
class TestRustFormat:
    def test_no_change(self):
        code = 'fn main() {\n    println!("hello");\n}\n'
        snippet = RustSnippet(code, "rust:1.93", "test-rustfmt")
        assert snippet.format(compress=False) == code
